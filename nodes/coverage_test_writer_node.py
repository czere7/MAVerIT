...
def coverage_test_writer_node(agent_state: "AgentState") -> dict[str, str]:
    ...
    response = ModelWrapper().invoke(prompt, agent_state.get("run_id", ""), "coverage_test_writer_node")
    ...
    # write_log(f"[coverage_test_writer_node] Produced updated test class with {len(updated_test_class.splitlines())} line(s).", response.usage_metadata, agent_state)
